...
def display_momentum_tab(df, sector_data_dict, benchmark_data, enable_color_coding=True):
    ...
    # Warn if CMF data quality is suspect (all same values = no volume data)
    if "CMF" in df.columns:
        cmf_vals = df["CMF"].dropna()
        if len(cmf_vals) > 1 and cmf_vals.std() < 0.001:
            st.warning("⚠️ **CMF data unavailable** for sector indices (no volume data). Using RSI-only Z-score for Momentum Score. Switch to ETF Proxy (sidebar) for accurate CMF readings.")
        elif cmf_vals.isna().all():
            st.warning("⚠️ **CMF could not be computed** for any sector. Momentum Score is based on RSI only.")
...
def display_stock_screener_tab(analysis_date=None, benchmark_data=None, sector_data_dict=None, momentum_weights=None, df_momentum=None):
    ...
    # Prominent date display - shows which date the screener is running for
    weekday_name = selected_date.strftime("%A")
    st.success(f"\U0001f4c5 **Screener Analysis Date:** {selected_date.strftime('%d %b %Y')} ({weekday_name})")
    ...
    for idx, (sector, symbol, name) in enumerate(universe):
        ...
        try:
            ...
            # Get price at selected date (use ffill to handle non-trading days)
            daily_dates = daily.index.normalize().date if hasattr(daily.index, "normalize") else daily.index.date
            # Find the row corresponding to selected_date or the last available row
            sel_date_ts = pd.Timestamp(selected_date)
            date_mask = daily.index <= sel_date_ts + pd.Timedelta(days=1)
            daily_to_date = daily[date_mask]
            if daily_to_date.empty:
                daily_to_date = daily
            price = float(daily_to_date["Close"].iloc[-1])
            sma8 = daily_to_date["Close"].rolling(8).mean().iloc[-1]
            sma20 = daily_to_date["Close"].rolling(20).mean().iloc[-1]
            sma50 = daily_to_date["Close"].rolling(50).mean().iloc[-1]
    ...
    st.markdown(f"#### ðŸŸ¢ Top 10 Bullish — {selected_date.strftime('%d %b %Y')}")
    ...
    st.markdown(f"#### ðŸ”´ Top 10 Bearish — {selected_date.strftime('%d %b %Y')}")
